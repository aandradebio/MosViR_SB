...
input.csv4 <- args[3] #"featureMatrix_arboviruses_500bp_12_test.csv.gz"
input.csv5 <- args[4] #"featureMatrix_mosquito_500bp_12_test.csv.gz"
...
class1_test$class <- replicate(nrow(class1_test), "arboviruses")
class2_test$class <- replicate(nrow(class2_test), "mosquito")
...
for (i in 1:50){
    ...
    # Save trained model
    save(rf, file = paste0("./", args[5], "/model_rf_", i, ".rda"))
...
write.csv(AUC, file=paste0(args[5],"/AUC.txt"), row.names=F)
write.csv(overall_rf, file=paste0(args[5],"/Accuracy.txt"), row.names=F)
write.csv(byClass_rf, file=paste0(args[5],"/byClass.txt"), row.names=F)
