...
tab <- fread(paste("./featureMatrices_knownContigs_", args[1], "_12_step2.csv", sep = ""), header = TRUE)
tab <- as.data.frame(tab)
tab <- tab[, !names(tab) %in% "seqid", drop = FALSE]
tab$classes <- factor(tab$classes, levels = c("arboviruses", "mosquito"))

cat("Loading model...\n")
load(paste0("/labinfo/apps/labinfo/R/4.3.0_bio/lib64/R/library/MosVir/models/models_class2_",args[1],"_12.rda"))
...
roc_obj <- roc(tab$classes, 1 - pma[, "arboviruses"], levels = c("mosquito", "arboviruses"))
auc_score <- auc(roc_obj)
predictions <- ifelse(pma[, "arboviruses"] > 0.5, "arboviruses", "mosquito")
...
rm(models)
save.image(paste0(args[1],"_metrics_Class2_knownContigs.RData"))
...
#write.csv(results,"metrics_Class2.csv", row.names=F, append=T)
