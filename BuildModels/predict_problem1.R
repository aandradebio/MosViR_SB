...
class1$class <- replicate(nrow(class1), "mosquito.associated")
class2$class <- replicate(nrow(class2), "mosquito.associated")
class3$class <- replicate(nrow(class3), "otherviruses")

test <- rbind(class1,class2)
test <- rbind(test,class3)
...
for (i in 1:100){
    ...
    load(paste0(args[4],"/model_rf_",i,".rda"))
